...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges.append((OBS0,PAC))
    edges.append((PAC,OBS1))
    edges.append((OBS1,GHOST1))
    
    variableDomainsDict[GHOST0] = [(x_val, y_val) for x_val in range(X_RANGE) for y_val in range(Y_RANGE)]
    variableDomainsDict[OBS0] = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
    variableDomainsDict[PAC] = [(x_val, y_val) for x_val in range(X_RANGE) for y_val in range(Y_RANGE)]
    variableDomainsDict[OBS1] = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
    variableDomainsDict[GHOST1] = [(x_val, y_val) for x_val in range(X_RANGE) for y_val in range(Y_RANGE)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            factors_no_var, factors_var = joinFactorsByVariable(factor, variable)
            if variable in queryVariables or len(factors_var.unconditionedVariables()) > 1:
                factors_no_var.append(eliminate(factors_var, variable))
            factor = factors_no_var
        return normalize(joinFactors(factor))
